...
class JsonLoader:
    ...
    def _gate_config_to_gate(self, gate_config: dict[str, Any]) -> "Gate":
        ...
        gate_definition = {
            ...
            "parent_stage": gate_config.get("parent_stage", ""),
            "locks": gate_config.get("components", [])
        }

        # Import at runtime to avoid circular imports
        from stageflow.gates import Gate
        return Gate(gate_definition)
    ...
    def _parse_gate(self, gate_name: str, gate_def: dict[str, Any]) -> "Gate":
        ...
        try:
            ...
            gate_definition = {
                ...
                "parent_stage": gate_def.get("parent_stage", ""),
                "locks": gate_def.get("locks", [])
            }
    ...
    def _process_to_dict(self, process: "Process") -> dict[str, Any]:
        ...
        for stage in process.stages:
            ...
            for gate in stage.gates:
                ...
                # Extract locks from gate's _locks attribute
                for lock in gate._locks:
                    lock_data = {
                    ...
                    if hasattr(lock, 'validator_name') and lock.validator_name:
