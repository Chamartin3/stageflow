...
def load_process_from_source(source: str, verbose: bool = False):
    ...
    if source_type == SourceType.FILE:
        ...
        if verbose:
            show_progress(f"Loading process from registry: {process_name}", verbose=True)
        ...
        try:
            ...
            if not process_file_path or not process_file_path.exists():
                raise typer.BadParameter(f"Process '{process_name}' not found in registry")

            return load_process_graceful(process_file_path)
        except Exception as e:
            raise typer.BadParameter(f"Failed to load process from registry: {e}") from e
