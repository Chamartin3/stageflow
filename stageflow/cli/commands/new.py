...
def new_command(
    file_path: Annotated[str | None, typer.Argument(help="File path for new process")] = None,
    template: Annotated[str, typer.Option("--template", "-t", help="Template type")] = "basic",
    ...
    if file_path is None:
        ...
        console.print("Create a new process file from template (always generates YAML).\n")
        console.print("[cyan]Arguments:[/cyan]")
        console.print("  FILE_PATH  File path for new process (will add .yaml if missing) [required]\n")
        console.print("[cyan]Options:[/cyan]")
        available_templates = "|".join(ProcessTemplate.list_templates())
        console.print(f"  --template, -t  Template type ({available_templates}) [default: basic]")
    ...
    try:
        ...
        if not ProcessTemplate.is_valid(template):
            ...
            raise typer.BadParameter(f"Invalid template '{template}'. Available: {available}")
